...
const AddDocumentVerification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiService.documentVerifications.create(submitData);
      toast.success('Document verification created successfully');
      navigate('/document-verifications');
    } catch (error) {
      console.error('Error creating document verification:', error);
      toast.error(error.response?.data?.message || 'Failed to create document verification');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h2>Add New Document Verification</h2>
        <p>Create a new document verification record</p>
      </div>
      ...
      <div className="info-note">
        <p><strong>Note:</strong> Turnaround time and status will be automatically calculated based on the dates and expected processing days.</p>
      </div>
    </div>
  );
};

export default AddDocumentVerification;
